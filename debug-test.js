...
describe('Debug Conversation History', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        mockBridge = new MockOllamaMCPBridge();

        // Setup default mock implementations
        mockBridge.callTool = jest.fn().mockResolvedValue('Mocked weather data for requested location');
        mockBridge.chatWithOllama = jest.fn().mockResolvedValue('Mocked weather response from Ollama');
        mockBridge.processWithTools = jest.fn().mockResolvedValue('Mocked processWithTools response');
