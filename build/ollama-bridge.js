...
When the user asks for something that requires these tools, use them and provide the results.`
    ...
    async analyzeToolNeeds(userMessage, tools) {
        const toolDescriptions = tools.map(tool => `- ${tool.name}: ${tool.description}\n  Parameters: ${JSON.stringify(tool.inputSchema?.properties || {}, null, 2)}`).join('\n');
...
11. Always respond with valid JSON only`;
     ...
     */
    async fallbackToolSelection(userMessage) {
        ...
        if (message.includes('url') || message.includes('link') || message.includes('shorten') ||
            ...
            if (message.includes('shorten'))
                operation = 'shorten';
            else if (message.includes('expand'))
                operation = 'expand';
    ...
    async processWithTools(userMessage, model) {
        try {
            // Get available tools
...
//# sourceMappingURL=ollama-bridge.js.map