...
Remember to interpret the weather data and provide helpful advice based on conditions.`;
        ...
        super(
            'WeatherAgent',
            'Specialized agent for weather information, forecasts, and weather-related advice',
            systemPrompt,
            ['weather_info', 'get_datetime'], // Can also use datetime for weather timing
            bridge
        );
    }
     ...
     */
    private async analyzeWeatherRequest(
        message: string,
        context: AgentContext
    ): Promise<{
        ...
        // Determine units
        let units: 'metric' | 'imperial' | 'kelvin' = 'metric';
        if (lowerMessage.includes('fahrenheit') || lowerMessage.includes('°f') || lowerMessage.includes('imperial')) {
            ...
            if (isUSLocation) units = 'imperial';
        }
...
Please provide a helpful response based on the weather information:`;
     ...
     */
    async getTravelWeatherAdvice(
        location: string,
        conversationId: string = 'travel'
    ): Promise<{
        response: string;
        toolsUsed: string[];
        context: AgentContext;
